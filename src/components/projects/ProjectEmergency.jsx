...
const procedureTypes = [
  { id: 'medical', label: 'Medical Emergency', icon: Stethoscope, defaultSteps: ['Cease flight operations', 'Ensure scene safety', 'Call 911 if serious', 'Administer first aid', 'Document incident'] },
  { id: 'fire', label: 'Fire Emergency', icon: Flame, defaultSteps: ['Alert personnel - evacuate', 'Call 911', 'Only fight small fires if trained', 'Account for all at muster point'] },
  { id: 'aircraft', label: 'Aircraft Incident', icon: Plane, defaultSteps: ['Note last position', 'Do not approach if fire', 'Secure the area', 'Do not disturb wreckage', 'Report to FIC if fly-away'] },
  { id: 'weather', label: 'Severe Weather', icon: AlertTriangle, defaultSteps: ['Land aircraft immediately', 'Seek shelter', 'Wait 30 min after lightning', 'Resume only when safe'] }
]

// ============================================
// MAP PREVIEW
// ============================================
function MapPreview({ siteLocation, boundary, musterPoints, evacuationRoutes, height = 200, onEdit }) {
      ...
      if (Array.isArray(musterPoints)) {
        musterPoints.forEach(mp => {
          if (mp.coordinates?.lat) {
            L.marker([mp.coordinates.lat, mp.coordinates.lng], { icon: icon('#f59e0b', '🚨', 28) }).addTo(map)
          }
        })
      }
  ...
  }, [siteLocation, boundary, musterPoints, evacuationRoutes])

  const hasContent = (Array.isArray(musterPoints) && musterPoints.length > 0) || (Array.isArray(evacuationRoutes) && evacuationRoutes.length > 0)
      ...
      {!hasContent && !loading && (
        <div className="absolute inset-0 flex items-center justify-center bg-gray-50/80 pointer-events-none" style={{ zIndex: 5 }}>
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500">No muster points set</p>
          </div>
        </div>
      )}
    </div>
...
// MAP EDITOR MODAL
// ============================================
function EmergencyMapEditor({ siteLocation, boundary, musterPoints, evacuationRoutes, onSave, isOpen, onClose }) {
  const containerRef = useRef(null)
  const mapRef = useRef(null)
  ...
  const [mode, setMode] = useState('muster') // 'muster' or 'route'
    ...
    const init = async () => {
      ...
      if (siteLocation?.lat) {
        L.marker([siteLocation.lat, siteLocation.lng], { icon: createIcon('#1e40af', '📍', 24), opacity: 0.6 })
          .addTo(map).bindTooltip('Site Location')
      }
      ...
      // Existing muster points (draggable)
      if (Array.isArray(musterPoints)) {
        musterPoints.forEach((mp, idx) => {
          if (mp.coordinates?.lat) {
            const m = L.marker([mp.coordinates.lat, mp.coordinates.lng], {
  ...
  useEffect(() => {
    ...
    // Remove old temp points
    Object.keys(markersRef.current).forEach(key => {
      if (key.startsWith('tempPt_')) {
    ...
    if (tempRoutePoints.length >= 2) {
      const line = L.polyline(tempRoutePoints.map(p => [p.lat, p.lng]), {
        color: '#f87171', weight: 3, dashArray: '5,5'
      }).addTo(map)
    ...
    tempRoutePoints.forEach((pt, i) => {
      const vm = L.circleMarker([pt.lat, pt.lng], {
        radius: 6, color: '#ef4444', fillColor: 'white', fillOpacity: 1, weight: 2
      }).addTo(map)
  ...
  const saveRoute = () => {
    ...
    const newRoute = {
      name: `Route ${localRoutes.length + 1}`,
      description: '',
    ...
    Object.keys(markersRef.current).forEach(key => {
      if (key.startsWith('tempPt_') && mapRef.current) {
      ...
      <div className="bg-white rounded-xl w-full max-w-5xl max-h-[95vh] flex flex-col shadow-2xl">
        <div className="p-4 border-b flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500">Set muster points and evacuation routes</p>
          </div>
        ...
        <div className="p-3 border-b flex flex-wrap gap-2 items-center">
          ...
          <button
            onClick={() => { setMode('muster'); setIsDrawingRoute(false) }}
            className={`px-3 py-1.5 rounded-lg text-xs flex items-center gap-1 ${mode === 'muster' && !isDrawingRoute ? 'bg-amber-500 text-white' : 'bg-gray-100 hover:bg-gray-200'}`}
          >
            🚨 Add Muster Point
          </button>
          <button
            onClick={startDrawingRoute}
            className={`px-3 py-1.5 rounded-lg text-xs flex items-center gap-1 ${isDrawingRoute ? 'bg-red-600 text-white' : 'bg-gray-100 hover:bg-gray-200'}`}
          >
            <Route className="w-3 h-3" /> {isDrawingRoute ? 'Drawing Route...' : 'Draw Route'}
          </button>
          {isDrawingRoute && (
            <>
              {tempRoutePoints.length >= 2 && (
                <button onClick={saveRoute} className="px-2 py-1 text-xs bg-green-100 text-green-700 hover:bg-green-200 rounded">
                  Save Route ({tempRoutePoints.length} pts)
                </button>
              )}
          ...
          <span className="ml-4 text-xs text-gray-500">
            🚨 {localMusterPoints.length} muster • 🛣️ {localRoutes.length} routes
          </span>
        </div>
...
export default function ProjectEmergency({ project, onUpdate }) {
  ...
  const emergencyPlan = useLegacy 
    ? (project.emergencyPlan || {})
    : (activeSite?.emergency || {})
    ...
    if (useLegacy && !project.emergencyPlan) {
      onUpdate({
        emergencyPlan: {
          ...
          procedures: procedureTypes.map(p => ({ id: p.id, label: p.label, steps: p.defaultSteps })),
          musterPoints: [],
          evacuationRoutes: [],
  ...
  // Muster points (text only - map handles coordinates)
  ...
  const handleMapSave = ({ musterPoints, evacuationRoutes }) => {
    updateEmergencyPlan({ musterPoints, evacuationRoutes })
  }
    ...
    <div className="space-y-6">
      ...
      {!useLegacy && (
        <div className="text-sm text-gray-500 flex items-center gap-2">
          <MapPin className="w-4 h-4" /> Emergency Plan for: <span className="font-medium text-gray-700">{activeSite?.name}</span>
        </div>
      )}
      ...
      <div className="card">
        <button onClick={() => toggleSection('muster')} className="flex items-center justify-between w-full text-left">
          <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
            <Navigation className="w-5 h-5 text-blue-600" /> Muster Points & Evacuation Routes
          </h2>
      ...
      <div className="card">
        <button onClick={() => toggleSection('contacts')} className="flex items-center justify-between w-full text-left">
          <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
            ...
            <span className="px-2 py-0.5 text-xs bg-gray-100 rounded">{(emergencyPlan.contacts || []).length}</span>
          </h2>
        ...
        {expandedSections.contacts && (
          <div className="mt-4 space-y-3">
            ...
            <button onClick={addContact} className="w-full p-3 border-2 border-dashed border-gray-300 rounded-lg text-gray-500 hover:border-blue-400 hover:text-blue-600 flex items-center justify-center gap-2">
        ...
        {expandedSections.hospital && (
          <div className="mt-4 grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={emergencyPlan.hospital?.name || ''} onChange={e => updateEmergencyPlan({ hospital: { ...emergencyPlan.hospital, name: e.target.value } })} className="input" placeholder="e.g., Squamish General Hospital" />
            </div>
            <div>
              <label className="label">Phone</label>
              <input type="text" value={emergencyPlan.hospital?.phone || ''} onChange={e => updateEmergencyPlan({ hospital: { ...emergencyPlan.hospital, phone: e.target.value } })} className="input" placeholder="e.g., 604-892-5211" />
            </div>
            <div className="sm:col-span-2">
              <label className="label">Address</label>
              <input type="text" value={emergencyPlan.hospital?.address || ''} onChange={e => updateEmergencyPlan({ hospital: { ...emergencyPlan.hospital, address: e.target.value } })} className="input" placeholder="Full address" />
            </div>
            <div>
              <label className="label">Distance</label>
              <input type="text" value={emergencyPlan.hospital?.distance || ''} onChange={e => updateEmergencyPlan({ hospital: { ...emergencyPlan.hospital, distance: e.target.value } })} className="input" placeholder="e.g., 15 km" />
            </div>
            <div>
              <label className="label">Drive Time</label>
              <input type="text" value={emergencyPlan.hospital?.driveTime || ''} onChange={e => updateEmergencyPlan({ hospital: { ...emergencyPlan.hospital, driveTime: e.target.value } })} className="input" placeholder="e.g., 20 minutes" />
            </div>
          </div>
        )}
      </div>
      ...
      <div className="card">
        <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2 mb-4">
          <Plus className="w-5 h-5 text-blue-600" /> First Aid
        </h2>
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label className="label">First Aid Kit Location</label>
            <input type="text" value={emergencyPlan.firstAid?.kitLocation || ''} onChange={e => updateEmergencyPlan({ firstAid: { ...emergencyPlan.firstAid, kitLocation: e.target.value } })} className="input" placeholder="e.g., In project vehicle" />
          </div>
